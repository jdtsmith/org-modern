...
(defun org-modern--progress ()
  ...
  (let* ((beg (match-beginning 1)) (end (match-end 1))
         (beg-stat (1+ beg)) (end-stat (1- end))
         (stat-len (- end-stat beg-stat))
         (completed (string-to-number (or (match-string 2) (match-string 3))))
         (bar-width (min org-modern-progress
                         (/ (float (* completed org-modern-progress))
			    (if (match-beginning 2) 100 ; percentage
			      (max 1 (string-to-number (match-string 4)))))))
         (ideal-pad (max 0 (/ (float (- org-modern-progress stat-len)) 2)))
         (cmpl-chars (min stat-len (max 0 (round (- bar-width ideal-pad)))))
